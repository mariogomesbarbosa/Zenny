...
| 6 | **O PR que criou os agentes não passou pelo `juiz`.** Agentes só são carregados no início da sessão, então o `juiz` não existia na sessão que o escreveu | As checagens que ele faria foram feitas à mão: fronteiras por `grep` (o núcleo não toca DOM, o `app.js` não soma dinheiro), `.gitignore` por `git check-ignore`, numeração do fluxo por leitura. Vale uma passada do `juiz` de verdade numa sessão nova |
